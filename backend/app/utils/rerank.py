...
def rerank_nodes(
    ...
    """条件触发式重排的实现，返回 ``(处理后的 nodes, 是否真的重排过)``。

    第二个返回值存在的原因：这个函数有好几条"不重排、原样返回"的路径
    （rerank 关闭、候选为空、top1 已经够高、候选数不够多），走这些路径时
    nodes 上带的仍然是**融合(RRF)后的分数**，量级在 0.026~0.033，跟重排后
    cross-encoder 分数（0.01~0.99，区分度很好）完全不是一回事。

    调用方如果要拿 ``nodes[0].score`` 去比某个按 cross-encoder 标定的阈值
    （``handlers/auto_router.py`` 的 ``AUTO_ROUTE_SCORE_THRESHOLD`` 就是），
    必须先知道这次到底有没有真的重排——否则会拿一个 0.03 量级的 RRF 分去比
    0.6，恒定判负。这个 bug 真实发生过：同一个问题"图书馆怎么借书？"在两次
    请求里分别得到 top1=0.94（重排过）和 top1=0.01（没重排），后者被判成
    "检索置信度不足"甩给 Agent，慢了四倍，而且给用户看的路由理由是假的。

    只关心结果、不关心是否重排的调用方用 ``ConditionalRerankPostprocessor``
    就行（它是标准 ``BaseNodePostprocessor``，能直接挂进 query engine）。
    """
    import configs.load_env as load_env

    if not load_env.RERANK_ENABLED:
        return nodes[: load_env.RERANK_TOP_N], False
...
class ConditionalRerankPostprocessor(BaseNodePostprocessor):
    """``rerank_nodes`` 的 ``BaseNodePostprocessor`` 包装，可直接挂进 query engine。

    只返回节点，丢掉"是否真的重排过"这个标志——需要那个标志的调用方直接用
    ``rerank_nodes``，见该函数 docstring。
    """
    ...
    def _postprocess_nodes(
        ...
        return rerank_nodes(nodes, query_bundle)[0]
