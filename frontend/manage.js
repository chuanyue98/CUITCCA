...
async function loadIndexSummary(indexName) {
    const textarea = document.getElementById('index-summary-textarea');
    const statusTag = document.getElementById('summary-status-tag');
...
// 索引摘要防抖保存
function debouncedUpdateSummary(text) {
    if (!currentActiveIndex) {
...
function renderNodesPage() {
    ...
    pageItems.forEach(node => {
        ...
        card.querySelector('.node_doc_id').textContent = node.doc_id || '自动生成';

        const textarea = card.querySelector('.node_editor');
        textarea.value = node.text || '';

        // 绑定输入事件 (防抖)
        const statusTag = card.querySelector('.node_status_tag');
        textarea.addEventListener('input', (e) => {
            debouncedUpdateNode(node.node_id, e.target.value, statusTag);
        });

        // 绑定按钮事件
        card.querySelector('.btn-delete-doc').onclick = () => deleteDocByCard(node.doc_id);
        card.querySelector('.btn-delete-chunk').onclick = () => deleteNodeByCard(node.node_id);
